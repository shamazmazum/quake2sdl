...
//    glScalef (frame->scale[0], frame->scale[1], frame->scale[2]);
    ...
    //                    qglColor4f (l* shadelight[0], l*shadelight[1], l*shadelight[2], alpha);
            ...
            while (1)
            {
                    ...
                    do
                    {
                        ...
                        if (!(currententity->flags & RF_TRANS_ADDITIVE))
                        {
                            ...
                            if (currententity->renderfx & RF2_CAMERAMODEL && currententity->flags & RF_TRANSLUCENT)
                            {
                                ...
                                VectorSubtract(r_newrefdef.vieworg, s_lerped[index_xyz], vert_len);
                                thisalpha *= VectorLength(vert_len)/cl_3dcam_dist->value;
                                
                                if (thisalpha>alpha)
                                    thisalpha = alpha;
                            }
                        
                            qglColor4f( shadelight[0], shadelight[1], shadelight[2], thisalpha);
                        }
                ...
                else
                {
                    do
                    {
                        ...
                        VectorScale(shadelight, l, lightcolor);
                        ...
                        if (!(currententity->flags & RF_TRANS_ADDITIVE))
                        {
                            ...
                            if (r_cellshading->value && !(currententity->flags&RF_TRANSLUCENT))
                            {
                                float highest = 0;
                                float cellcolors[16] = {
                                    0.1, 0.1,
                                ...
                                for (i=0; i<3; i++)
                                {
                                    lightcolor[i] = cellcolors[(int)(lightcolor[i]*16.0f)];
                            ...
                            qglColor4f (lightcolor[0], lightcolor[1], lightcolor[2], thisalpha);
                        }
        ...
        if (r_cellshading->value)
            GL_DrawOutLine (paliashdr, currententity->frame, r_cellshading_width->value);
    }
...
void GL_DrawAliasShadow (dmdl_t *paliashdr, int posenum, qboolean mirrored)
{
//    msurface_t *surf;
    dtrivertx_t    *verts;
    int        *order;
    vec3_t    temp[4], point, angles, vectors[3], forward, right, up;
    float    height, lheight;
    int        count;
    daliasframe_t    *frame;

    VectorSet(forward,        0.5, 0.5, 0);
    ...
    while (1)
    {
        ...
        do
        {
            if (true) //relative origin/angle -- faster...
            {
            ...
            else //absolute origin/angle -- much slower...
            {
                memcpy( temp[0], s_lerped[order[2]], sizeof( temp[0] )  );

                RotatePointAroundVector( temp[1], forward,    temp[0], angles[ROLL] );
                RotatePointAroundVector( temp[2], up,        temp[0], angles[YAW] );
                RotatePointAroundVector( temp[3], right,    temp[2], angles[PITCH] );

                VectorAdd(temp[3], currententity->origin, point);

                point[0] -= shadevector[0]*(point[2]+lheight);
                point[1] -= shadevector[1]*(point[2]+lheight);
                point[2] = lightspot[2] + 0.1f;
            }
...
void R_DrawAliasModel (entity_t *e)
{
    ...
    vec3_t        bbox[8];
    image_t        *skin;
    qboolean    mirrormodel = currententity->flags&RF_MIRRORMODEL || ( r_lefthand->value == 1.0F &&
...
// =================
    ...
    if (currententity->flags&RF_TRANS_ADDITIVE && currententity->flags & RF_TRANSLUCENT)
    { 
        ...
        default:
            {
                ...
                GL_DrawAliasShadow( paliashdr, currententity->frame, mirrormodel );
