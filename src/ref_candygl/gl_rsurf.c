	...
	*/
	if (!gl_lightmap->value)
	{
		...
		else
		{
			...
			{
				switch ( gl_monolightmap->string[0] )
				{
				case 'I': case 'i':
...
void R_DrawAlphaSurfaces (void)
{
	...
	for (s=r_alpha_surfaces ; s ; s=s->texturechain)
	{
		...
		if (s->texinfo->flags & SURF_TRANS33 && s->texinfo->flags & SURF_TRANS66)
			qglColor4f (intens,intens,intens, DIV254BY255);
...
void RenderPolyFunc (int nv, float *v, float scroll)
{
	float *poly;
	int i;

	//GL_MODULATE, GL_DECAL, GL_BLEND, GL_REPLACE_EXT, GL_ADD
//	GL_TexEnv( GL_ADD );
